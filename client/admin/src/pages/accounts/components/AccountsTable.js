...
const columns = [
  ...
  { Header: "info", accessor: "info", width: "23%", align: "left" },
...
function AccountsTable({ accounts, filters, deleteAccount }) {
  ...
  const handleEdit = (id) => {
    changeCloneAccount(accounts.find((a) => a.id === id) || null);
    toggleModal(true);
  };
  const handleDelete = (id) => {
    const response = delAccount(id);
    if (response.status === 200) {
      deleteAccount(id);
      handleSuccess("Xoá người dùng thành công!");
    } else {
      handleError("Xoá người dùng thất bại!");
    }
  };
  const getAccountRow = (account) => {
    return {
      id: (
        <MDBox sx={{ width: 150, whiteSpace: "nowrap" }}>
      ...
      info: (
        <MDBox display="flex" alignItems="center" lineHeight={1}>
          <MDBox lineHeight={1}>
            ...
            <MDTypography variant="caption">{account.email}</MDTypography>
          </MDBox>
        </MDBox>
      ),
      ...
      status: (
        <MDBox ml={-1}>
          <MDBadge
            ...
            color={account.status.toString().toLowerCase() === "active" ? "success" : "error"}
            variant="gradient"
            size="sm"
          />
        </MDBox>
      ...
      tempAccounts = tempAccounts.filter((a) =>
        filters.hasOwnProperty("keyword") && filters.keyword != null
          ? a.name.toLowerCase().includes(filters.keyword.toLowerCase()) ||
            a.email.toLowerCase().includes(filters.keyword.toLowerCase()) ||
            a.username.toLowerCase().includes(filters.keyword.toLowerCase()) ||
            a.phone.toString().toLowerCase().includes(filters.keyword.toLowerCase())
          : true
      );
      tempAccounts = tempAccounts.filter((a) =>
